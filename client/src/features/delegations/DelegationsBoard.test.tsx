...
describe('DelegationsBoard', () => {
  ...
  it('names the model a task actually runs, not the Claude one at that rung', async () => {
    ...
    expect(within(grok).getByText('grok-4.7 →')).toBeInTheDocument()
  ...
  it('board defaults to the fleet envelope', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Board scope' })).toBeInTheDocument()
    expect(await screen.findAllByText('Antiphon row')).not.toHaveLength(0)
    expect(screen.getAllByText('gym-stat row').length).toBeGreaterThan(0)
  ...
  it('board selection scopes rows and counters', async () => {
    ...
    renderWithProviders(<DelegationsBoard />)
    await screen.findAllByText('Antiphon row')
    await userEvent.click(screen.getByRole('textbox', { name: 'Board scope' }))
    await userEvent.click(await screen.findByRole('option', { name: 'Antiphon board (Antiphon)' }))
    await waitFor(() => expect(screen.queryAllByText('gym-stat row')).toHaveLength(0))
    expect(screen.getAllByText('Antiphon row').length).toBeGreaterThan(0)
    ...
    await waitFor(() => expect(screen.queryAllByText('Antiphon row')).toHaveLength(0))
    expect(screen.getAllByText('gym-stat row').length).toBeGreaterThan(0)
    await userEvent.click(screen.getByRole('textbox', { name: 'Board scope' }))
    await userEvent.click(await screen.findByRole('option', { name: 'All boards' }))
    await waitFor(() => expect(screen.getAllByText('Antiphon row').length).toBeGreaterThan(0))
    expect(screen.getAllByText('gym-stat row').length).toBeGreaterThan(0)
  ...
  it('board shows exact exclusions', async () => {
    ...
    expect(screen.queryAllByText('Antiphon row')).toHaveLength(0)
    expect(screen.queryAllByText('gym-stat row')).toHaveLength(0)
  })
})
