...
public class DelegationSequencingE2ETests
{
    ...
    private sealed class TestSessionRunner(AntiphonAppFixture fixture)
    {
        ...
        public Dictionary<string, string> EnvFor(Guid sessionId, Guid taskId, string? token)
        {
            ...
            {
                // Neutralise nested-Claude markers: these tests usually run from inside a Claude
                // session, and a child that sees them does not persist its transcript.
    ...
    /// <summary>A scratch git repo carrying the real skill and script, discovered from cwd.</summary>
    private sealed class ScratchRepo : IDisposable
    {
        ...
        public ScratchRepo()
        {
            Path = Directory.CreateTempSubdirectory("antiphon-sequencing-e2e").FullName;
            ...
            File.WriteAllText(System.IO.Path.Combine(Path, "README.md"), "# Scratch workspace\n");
            Git("init");
            Git("add", ".");
