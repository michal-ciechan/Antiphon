...
public class WorkflowDefinitionLoaderTests
{
    ...
    // CARD-0175: `issue.*` is the TRACKER's view of the card and `card.*` is Antiphon's. They were
    // the same string while an imported card's Identifier WAS the tracker key; now they diverge,
    // and `Work on {{ issue.identifier }}` on an existing board must keep naming the GitHub issue.
    [Test]
    public void Prompt_variables_split_the_tracker_key_from_the_card_identifier_when_linked()
    {
        var card = new Card
        {
            Id = Guid.NewGuid(),
        ...
        var variables = WorkflowDefinitionLoader.BuildPromptVariables(card, worktree: null);

        variables["issue.identifier"].ShouldBe("#3");
        variables["card.identifier"].ShouldBe("CARD-0176");
        variables["issue.url"].ShouldBe("https://github.test/acme/app/issues/3");
        variables["issue.tracker"].ShouldBe("GitHub");
        variables["issue.reference"].ShouldBe(" (GitHub issue #3, https://github.test/acme/app/issues/3)");

        WorkflowDefinitionLoader.RenderPrompt(
            "Work on card {{ card.identifier }}{{ issue.reference }}: {{ issue.title }}",
            variables)
            .ShouldBe("Work on card CARD-0176 (GitHub issue #3, https://github.test/acme/app/issues/3): Imported work");
    }

    [Test]
    public void An_unlinked_card_renders_both_identifiers_the_same_and_no_reference_clause()
    {
        var card = new Card
        {
            Id = Guid.NewGuid(),
        ...
        variables["issue.identifier"].ShouldBe("CARD-0007");
        variables["card.identifier"].ShouldBe("CARD-0007");
        variables["issue.url"].ShouldBeNull();
        variables["issue.tracker"].ShouldBeNull();
        variables["issue.reference"].ShouldBe(string.Empty);

        // The default template must not render empty parentheses on an internal board.
        WorkflowDefinitionLoader.RenderPrompt(
            "Work on card {{ card.identifier }}{{ issue.reference }}: {{ issue.title }}",
            variables)
            .ShouldBe("Work on card CARD-0007: Filed by hand");
    }
