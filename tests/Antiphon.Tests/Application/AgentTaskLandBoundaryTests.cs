    ...
    public async Task C448_V10_EachAcknowledgedBoundaryRechecksSource(string boundary, string change, bool contained)
    {
        ...
        async Task MutateAsync()
        {
            ...
            if (change == "advance")
                ...
                await File.WriteAllTextAsync(Path.Combine(h.Fixture.Source, "keep.txt"), "new writer bytes\n");
            else
            {
                await using var db = h.CreateContext();
                var task = await db.AgentTasks.SingleAsync(t => t.Id == h.Fixture.TaskId);
                task.WorktreeBranch = "different-recorded-source";
        ...
        {
            if (boundary == "remote" && args[0] == "fetch" && result.Succeeded) await MutateAsync();
        };
        h.Fixture.Git.Trace.Clear();
        await h.RunAsync();
        ...
        op.LastReason.ShouldBe(change switch { "advance" => "source_changed", "dirty" => "source_dirty", _ => "task_coordinates_changed" });
        h.Fixture.Git.Trace.ShouldNotContain(a => a[0] == "push" || a.Contains("remove"));
        Directory.Exists(h.Fixture.Source).ShouldBeTrue();
        (await h.Fixture.RequiredAsync(h.Fixture.Source, "rev-parse", "HEAD")).Trim().ShouldBe(retainedSha);
        if (change == "dirty") (await File.ReadAllTextAsync(Path.Combine(h.Fixture.Source, "keep.txt"))).ShouldBe("new writer bytes\n");
        if (boundary is "remote" or "RebaseStarted")
