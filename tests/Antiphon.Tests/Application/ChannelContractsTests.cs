...
public class ChannelContractsTests
{
    ...
    [
        ...
        "",
        $"To send a file to the chat (PDF, image, document, ...), put {ChannelContracts.AttachMarkerFormat} on its own line anywhere in your reply, e.g. [[attach: C:\\work\\invoice.pdf]]. The marker line is removed from the delivered text and the file is sent as a document. Use absolute paths to files on this machine; up to 14 MB per turn. Document-producing tasks attach their Markdown sources automatically; a channel may opt into a conversion agent that adds a PDF. Slack shows HTML files as a text snippet, not a document.",
        "",
    ...
    public void Slack_preset_adds_threading_guidance_without_changing_the_shared_reply_contract()
    {
        ...
        slack.ShouldContain("Document-producing tasks attach their Markdown sources automatically");
