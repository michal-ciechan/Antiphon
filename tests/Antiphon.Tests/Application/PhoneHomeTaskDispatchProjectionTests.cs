...
public sealed class PhoneHomeTaskDispatchProjectionTests
{
    ...
    private static async Task VerifyProjectionAsync(AgentKind kind)
    {
        ...
        var dispatcher = CreateDispatcher(schema, host, sink).Dispatcher;
    ...
    public async Task Refused_runner_task_leaves_no_pushed_branch_or_mirror()
    {
        ...
        var sink = new RecordingLaunchSink();
        var git = new PushGit();
        var dispatcher = CreateDispatcher(schema, host, sink, git, allowDelegatedTasks: false).Dispatcher;
    ...
    private sealed record DispatchGraph(AgentTaskDispatcher Dispatcher, ServiceProvider Services);
