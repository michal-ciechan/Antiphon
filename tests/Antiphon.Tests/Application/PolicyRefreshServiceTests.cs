...
public class PolicyRefreshServiceTests
{
    ...
    [Arguments("notify")]
    [Arguments("off")]
    public async Task Phone_notify_or_working_keeps_loaded_stamp(string lane)
    {
        await using var h = await CreateHarnessAsync();
        var oldStamp = await SeedPhoneDriftAsync(h);
        if (lane == "working") await h.MarkWorkingAsync();
        else
        {
            var mode = lane == "off" ? PolicyRefreshMode.Off : PolicyRefreshMode.Notify;
            await using var db = CreateContext();
            await db.Agents.Where(a => a.Id == h.AgentId)
                .ExecuteUpdateAsync(u => u.SetProperty(a => a.PolicyRefreshMode, mode));
        }
        using var scope = h.Provider.CreateScope();
        ...
        {
            var force = lane == "working";
            var ex = await Should.ThrowAsync<ConflictException>(() => control.RefreshPolicyAsync(h.AgentId, force, default));
            ex.StatusCode.ShouldBe(409);
            ex.Code.ShouldBe(lane == "off"
                ? PolicyRefreshService.NotResumableCode
                : PolicyRefreshService.SessionWorkingCode);
        }
