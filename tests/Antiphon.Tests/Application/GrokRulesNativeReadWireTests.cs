...
public sealed class GrokRulesNativeReadWireTests
{
    [Test]
    [Timeout(180_000)]
    public async Task Calibrate_native_read_schema_and_capture_actual_tool_output(CancellationToken ct)
    {
        ...
        var root = Path.Combine(Path.GetFullPath("TestOutput/Logs/GrokRulesNativeReadWireTests"), Guid.NewGuid().ToString("N"));
        ...
        var body = string.Join("\r\n", Enumerable.Range(1, 1105).Select(n => $"RULE-LINE-{n:D4} neutral calibration material."));
        ...
        foreach (var arg in new[] { "--prompt-file", promptPath, "--output-format", "streaming-json", "--no-subagents",
            "--disable-web-search", "--always-approve", "--model", "grok-4.6", "--max-turns", "5", "--session-id", sessionId.ToString("D"),
