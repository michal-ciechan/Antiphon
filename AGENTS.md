...
- Restart, deploy and `dev-aspire.ps1` admit their source checkout by exact commit (CARD-0644), not by directory: HEAD must equal `-ExpectedSha <full-sha>` or, by default, the local `refs/remotes/origin/master` (never fetched by the script). Main and linked worktrees are treated alike, so a linked worktree at the admitted commit needs no override and a stale main checkout is refused (exit 3, nothing killed). They control the ONE shared local stack either way; its locks, wrapper PID, dashboard URL and `apphost.log` live in the main worktree's `logs/`. `-AllowWorktree` admits a linked worktree at its current HEAD with a warning and never overrides an explicit `-ExpectedSha`, unreadable Git, or the locks.
- `restart-apphost.ps1` builds from whatever is on disk in the main checkout; it does not fetch or pull. Pushing straight to `origin/master` from a worktree (a `-Land` workaround) never advances the main checkout's own `master` — the next restart then silently serves stale code while reporting healthy (CARD-0358; see the bootstrap doc's last gotcha). After any out-of-band push, `git pull --rebase` in the main checkout before restarting. Exit 0 now requires dashboard + `/health` + `/api/version` SHA equal to the admitted SHA (`-ExpectedSha`, alias `-ExpectedServerSha`); a mismatch is exit 5 with the restart lock retained. After a restart that matters, confirm `GET /api/version` (SHA and `land-v2`) rather than trusting a passing health check. A land confirms publication, not activation — see the Post-land server activation check in [docs/orchestration-loop.md](docs/orchestration-loop.md). More generally: check an assumption about running state directly and immediately — checkout HEAD, whether a restart picked up a change, whether a service is authenticated — never infer it from a downstream signal like "healthy" or "no errors logged."
