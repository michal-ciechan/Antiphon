# Antiphon.Messaging.FakeGateway

Kafka-connected fake messaging gateway for local dev and integration tests (dotnet tool
`antiphon-fake-gateway`). Records would-be deliveries, injects synthetic inbound messages
via `POST /inbound`, and simulates outages with `/pause`/`/resume`. Never deploy to
production.

This package is itself the smallest complete example of a gateway: a `FakeChannelAdapter`
hosted on `Antiphon.Messaging.Gateway`.
