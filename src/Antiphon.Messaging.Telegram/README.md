...
Telegram Bot API `IChannelAdapter` for the Antiphon messaging bus. Pair with
`Antiphon.Messaging.Gateway` to run a Telegram gateway, or with `Antiphon.Messaging.Service`
which already does that for the operator's own bots.
