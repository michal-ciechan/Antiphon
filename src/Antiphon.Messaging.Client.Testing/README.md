# Antiphon.Messaging.Client.Testing

In-memory fake of `Antiphon.Messaging.Client` for application tests: inject inbound
`ChannelMessage`s and observe produced `ChannelReply`s with no Kafka.

A gateway author's tests should use `Antiphon.Messaging.Gateway.Testing.InMemoryGatewayBus`
(in the Gateway package) instead — that is the other side of the bus.
