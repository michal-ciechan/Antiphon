...
Wire contracts for the Antiphon messaging bus: `ChannelMessage`, `ChannelReply`,
`ChannelCapabilities`, and `IChannelAdapter`. BCL only — no Kafka, no server, no Postgres.

A gateway implements `IChannelAdapter` and hosts it with `Antiphon.Messaging.Gateway`. An
application consumes the bus with `Antiphon.Messaging.Client`. Non-.NET integrators should
read `docs/messaging/contract/v1/CONTRACT.md` and the JSON Schema next to it.
...
- **Dependencies:** `Confluent.Kafka` floats within its major (`[2.6.0,3.0.0)`) where the
  package takes a Kafka dependency.
