...
public sealed class AgentTaskReplyService
{
        ...
        string? warning = null, string? drift = null, string? git = null)
    {
        ...
        var note = await BuildParentNoteAsync(task, report, workspaceNote, warning, drift, git, null, ct);
        try
        {
            await using var scope = _scopeFactory.CreateAsyncScope();
            var queue = scope.ServiceProvider.GetRequiredService<SessionMessageQueueService>();
    ...
    private async Task<DelegationReportFormatter.Note> BuildParentNoteAsync(
        AgentTask task, string report, string? workspaceNote, string? warning, string? drift, string? git,
        AppDbContext? settlementDb, CancellationToken ct)
    {
    ...
    {
        var note = await BuildParentNoteAsync(task, report, workspaceNote, warning, drift, git, db, ct);
