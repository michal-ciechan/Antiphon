...
SCOPE: Re-run the claimed ordinary checks (Unit plus named affected integration classes) before land. Executed PCs are not a prerequisite. Check the Code report's CP-n lines against the plan's ### Checkpoints table: a missing row, zero count, unlisted build/test run without a reason, a build or test driver outside the slot gate, a broad run without named invariant/cost, or a new test that cannot go red (self-compare, constant, no outcome assertion) is a defect.

ROUND: the brief's verification profile governs. A Final Review reruns the complete ordinary scope, incl. Interim-deferred rows. Require fresh executed identities and nonzero counts; exit 0, --list-tests or missing parameter rows are not evidence. Manual and PC checks stay pending; nightly green never satisfies them.
