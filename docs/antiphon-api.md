...
POST   /api/herdr/pane-disposals/preview    independent pane-addressed read-only inspection; requires paneId and at least one full expectedSessionId/expectedNativeSessionId UUID. Current increment always reports eligible=false and guardAvailable=false.
POST   /api/herdr/pane-disposals            { operationId, previewId, reason }; valid attempts durably refuse 409 herdr_disposal_guard_unavailable with operationId and receipt extensions. No teardown is implemented.
GET    /api/herdr/pane-disposals/{operationId} persisted refusal receipt, or 404. Runner equivalents are /herdr/pane-disposals (no /api). Script: scripts/herdr-pane.ps1 inspect|dispose|status; dispose requires -Execute to submit. Full guarded-backend disposal remains a CARD-0461 prerequisite; ordinary Stop still detaches.
