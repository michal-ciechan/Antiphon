...
| Runner provider sign-in (CARD-0628) | GET | `/api/session-runners/{runnerId}/provider-auth/{provider}` — `provider` is `claude`; a live probe on the runner returning `loggedIn`, `authMethod`, `subscriptionType`, `checkedAtUtc`, `error`. 409 `phone_home_unavailable` when the runner is not connected, 409 `phone_home_unsupported_operation` for a runner binary older than the probe. |
