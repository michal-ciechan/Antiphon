...
Driving a real V-7 turn needs three things the script does not yet do. (1) `SessionRunner:BaseUrl` must name an **isolated** runner, not the dead `http://127.0.0.1:1` guard: `AgentControlService` probes runner liveness for every standing start and turns an unreachable runner into 503 `standing_resume_runner_unavailable`, even though the pinned agent is dispatched over phone-home. Start `src/Antiphon.SessionRunner` on a random port with its own `SessionRunner__SessionLogPath`, as the E2E `IsolatedSessionRunner` does. (2) The phone-home epoch is counted independently on each side — the server from `AcceptConnect` since **its** process start, the runner from successful WebSocket connects since **its** process start — and both silently drop frames whose epoch differs. Restarting one side alone leaves the runner registered and `available=true` but permanently deaf: `lastHeartbeatUtc` never advances, `dispatchEligible` stays false, and `PhoneHomeRunnerClient.ListAsync` hangs. Restart the server **first**, then the container, so both count 1. (3) The first start of a standing Grok agent must be `fresh: true`; a resume is correctly refused because no native Grok history exists yet under `/state/grok/sessions`.
