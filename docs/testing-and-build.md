...
V-7 is still blocked after those three: the Linux pty-host binds `/tmp/antiphon-pty-<session>` and waits 30s with no Launch; the container runner's `NamedPipeClientStream.Connect` times out at 15s against that same socket (`srwxr-xr-x`, same `app` uid). Evidence: `.antiphon/card0490-live-evidence/container-logs.txt` and the host log under `pty-hosts-tmp`. Shadow copies live on overlay ext4 (`SessionRunner__PtyHostDir=/tmp/antiphon-pty-hosts`). Do not treat agent.Status=Running as a ready Linux host.
