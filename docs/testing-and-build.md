...
Exit codes: 0 green, 1 failed tests, 2 invalid manifest or build or missing/malformed TRX, 3 `minExecuted` or roster miss, 4 slot timeout, 5 row or total timeout, 6 the executor died, 7 owning task ended or could not be verified, 75 still running. The run exit is the highest of 2, 6, 4, 5, 1, 3, 0 for unbound runs; owner exit 7 takes precedence when bound ownership ends. Reruns happen only for names passed as `--known-flaky` (or `rerun.knownFlaky`), once, method-scoped, and the report prints `RERUN` lines with `reruns=1`. `--baseline <ref>` classifies each failure `INHERITED`, `INTRODUCED`, or `NEW` from a detached worktree of that commit. The report is `report.md` and `report.json` under the run folder. A red run keeps `failures.md`, the TRX, console logs, `host.txt`, and `git.txt`. A green run deletes that run's `bin-<name>/` directories. The executor never deletes its own shadow copy. `wait` deletes `<run>/tool/` once `phase=done` and the executor pid is gone; the next `start` sweeps `tool/` folders left by finished runs. `clean --run <id>` deletes a red run's outputs and never touches `bin/`, `obj/`, `workspace/`, or a `bin-*` name the manifest does not own.
