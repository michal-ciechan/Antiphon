...
When `Enabled` is true, each named read client's `HttpClient.Timeout` is infinite so the pipeline owns the deadline, including response bodies. Runner reads still pass `SessionRunner:RequestTimeoutSeconds` (default 100) as an owner deadline, so that setting remains an upper bound inside the 120-second total. `ListTimeoutSeconds` and the capability probe keep their shorter caps. When `Enabled` is false, that timeout stays the normal client timeout and the handler makes one attempt. A test factory whose client timeout is not infinite keeps the primary handler, which is how existing wire tests avoid the new pipeline. Dynamic settings are read when the client is created and on each request. R1 does not reload a live circuit; a restart applies configuration.
