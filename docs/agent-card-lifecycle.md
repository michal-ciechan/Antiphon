...
- **Rank** is derived at read time: `rank = 13 − (3·importance + 2·effectiveUrgency)`, lower sorts first. Board columns, orchestrator dispatch, the Home *Up next* rail and the `docs/cards/` index all sort by this. Ties break by `position` (placed cards first, dense `1..n` inside the cell; null = unplaced, last), then `dueAt` (earliest first, null last), then `CreatedAt`.
- **Position** (CARD-0098) is a stored ordinal inside the current (column, rank cell) only. It carries no tier information. Anything that changes the card's column or axes (`Importance` / `Urgency`) clears it; a `dueAt` change does not. Dispatch (`LoadEligibleCandidatesAsync`) reads the same key as the board. Write it with `PATCH /api/cards/{id}/position` or `card.ps1 reorder`.
