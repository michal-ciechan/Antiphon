...
- **D-9 — System git identity.** Bake `[user] name = Antiphon Runner` and
  ...
  **Amended by the operator for Round C (2026-09-23):** do not bake the identity. It is a
  non-secret file on server2, `~/antiphon-server2/secrets/gitconfig`, `[user] name =
  antiphon-server2-runner`, `email = antiphon-server2-runner@users.noreply.github.com`.
  `deploy-parent` creates it only when missing (values from `stack.env`
  `RUNNER_GIT_USER_NAME`/`EMAIL`, else those defaults) and never overwrites it; compose mounts it
  read-only at `/run/antiphon/gitconfig` and the entrypoint exports `GIT_CONFIG_GLOBAL` to it
  (global outranks system and replaces `~/.gitconfig`, so a stopgap cannot shadow it). Deploy
  removes a repository-local identity in the runner checkout and proves the effective origin.
...
| V-6 | `Gitconfig_sets_runner_identity_from_the_mounted_file` in `DindRunnerContractTests.cs` (amended D-9) | Baked gitconfig (comments ignored) and Dockerfile carry no identity; compose mounts `RUNNER_GIT_IDENTITY_FILE` read-only at `/run/antiphon/gitconfig` with `GIT_CONFIG_GLOBAL`; entrypoint refuses Missing/Unusable and exports it before the runner. Round C also adds `Deploy_parent_creates_the_git_identity_file_without_overwriting` in `RemoteScriptContractTests.cs` | No mount, no export |
...
| PC-12 | Drop the entrypoint's `export GIT_CONFIG_GLOBAL` (or the compose mount); variant: move the create outside `! -e` in `ensure_runner_git_identity` | V-6: export/mount missing; variant: `Deploy_parent_creates_the_git_identity_file_without_overwriting` write-outside-branch assertion |
