...
| Repository lease busy, cumulative wait still inside `Delegation:RunnerSyncBudgetSeconds` (R3/R4 amendment) | Unavailable / `runner_sync_lease_waiting`; **not settled**, task stays open | One attempt waits at most 10 s (`RemoteWorkspaceService.LeaseWaitSlice`, cut to the budget left) so a sweep is never held for the budget. The first busy sighting per task is kept in memory (`RunnerSyncLeaseWaits`; a restart restarts the wait) and the open-report sweep / delivery watchdog re-hand continues it. Once the cumulative wait reaches the budget the row above applies: `runner_sync_lease_busy`, Blocked. |
