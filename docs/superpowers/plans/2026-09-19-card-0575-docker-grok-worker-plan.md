...
VD-1 amendment: 2026-09-19, Plan task `0d678e51`, against TestDesign commit `c391749673d7b08bc37d5a5c7d52760543707e77`. D-14..D-17 and S4a..S4c resolve the refinement persistence seam. Its eight-control subtotal is preserved as historical accounting below.

Generation-provenance amendment: 2026-09-19, Plan task `5e3ba4d4`, source `3cfe11ae2c20943360e02f89b690c34ca11ef472` (TestDesign task `6b037196`). D-18/S4d select durable transcript provenance; D-19 approves PC-81's additional second-refinement receipt assertion; D-20 adopts G/PC-149..172 and the revised **203.4-minute Code / 536.7-minute Mutation floors**. The original 140 recipes and 174/413.5-minute baseline tables remain verbatim. The latest amendment at the end governs over the two historical review dispositions. **Next: test-design**, limited to the changed provenance/receipt seam, affected controls and cost reconciliation; Code is not yet commissioned.
...
| A transcript match can establish the generation that accepted the prompt. | At `3cfe11ae`, `TranscriptEntry`, `RunnerTranscriptEvent`/`RunnerTranscriptDto` and the server equivalents have no generation provenance. Queue confirm queries select text after a stored sequence or timestamp floor; `LastDeliveryGeneration` protects composer recovery, not the origin of that text. | D-18 carries the producing generation from a durable runner transcript boundary through ingestion and into the confirming receipt. Current session generation gates new input only. |
| Adding the current runner generation to every re-tailed row supplies that provenance. | `GrokTranscriptTailer.RunAsync` starts at byte zero, and `StartTailerFor`/`RestoreTailerFromSidecar` reopen the same native file on resume. `TranscriptSidecar` has no generation boundary and `SaveSidecar` is best-effort. The server rebases stored sequences and deduplicates by UUID/kind, with a sequence fallback. | Record generation byte ranges before a writer starts; re-tail assigns each source record from its original range. Unknown history remains unknown. Persist source identity separately from rebased display sequence; never fill a missing generation from the current session row. |
| PC-81's first-file assertion detects timestamp-only naming after immutable publication is added. | D-15's create-or-verify keeps A intact and holds B when their timestamp paths collide. The original first-file assertion can still pass. No mutation was executed in this Plan task. | D-19 keeps that assertion and additionally requires one complete receipt and exact file bytes for **both** accepted identities. A held B makes the second receipt-count assertion red. |
...
| D-18 | Stamp nullable `AcceptedStartedAt` and stable source-record identity on transcript entries from a durable, pre-launch Grok generation-range journal. Carry them unchanged over events/snapshots into PostgreSQL and save the matched tuple with the delivery verdict. This permits delayed original evidence while excluding identical replacement-generation text. | Current-session lookup, DTO-envelope generation applied to history, native row timestamps, process-start guesses and fixture-only labels cannot distinguish those histories. A fresh native conversation per generation would break standing-session continuity. |
| D-19 | Keep PC-81's timestamp-only mutation and first-file assertion; approve the additional second-refinement receipt assertion and both full-file checks specified below. They detect loss of an accepted instruction even when immutable-file validation prevents overwrite. | Weakening create-or-verify, changing the test to accept a permanent hold, or counting a timeout/exception as red would conceal the mutation. |
| D-20 | Adopt all 24 TestDesign additions G/PC-149..172, their arrangements and the 203.4/536.7-minute floors. Keep the earlier eight-row and original 140-row tables as labeled subtotals, and return D-18's new seams for a focused guard/cost audit. | Treating 148 as the final inventory, omitting FK-masked controls, or claiming the added provenance variants have already been audited/executed understates required verification. |
...
Recover transcript evidence **before** retrying input. Confirm only a complete immutable final wire `UserPrompt` in the original session/generation above that row's persisted attempt floor; validate the recorded artifact hashes, including both levels for a secondary spill. D-18 below specifies how that generation is proven and the confirming source tuple is persisted; neither the current generation nor `Sent` is evidence of a prompt's origin. Store its prompt sequence and `ConfirmedAt` idempotently. Screen evidence and an ID/header match cannot confirm the obligation. Qualification additionally observes the actual guest read of every referenced file and compares complete bytes; the runtime receipt flag alone is not proof of model file consumption. A crash between prompt, queue verdict and refinement confirmation uses late-confirm and adds no submit. Retain the original attempt baseline/generation during Enter-only recovery.
...
### D-18: durable producing-generation provenance

The generation remains the existing `(SessionId, AcceptedStartedAt)` equality token, normalized by `SessionGeneration.Normalize` to UTC microseconds. It is not the ACP row timestamp, `ChildStartUtc`, ingestion time, rules generation or the server's current `AgentSession.StartedAt`. This contract is required for Docker v0; null fields retain legacy host behavior. The unshipped `dockerGrokExecV1` capability includes this contract, so its existing launch gate must not advertise support for a partially implemented runner.

**Fields and owners.** Add the following optional wire/storage members; require the complete tuple for Docker receipt evidence. Generate the EF migration through the CLI, with null legacy values and no timestamp-based backfill.

| Surface | Added members and owner |
|---|---|
| `RunnerTranscriptEvent` and `SessionRunnerTranscriptEvent` | Trailing optional `DateTime? AcceptedStartedAt` and `TranscriptSource? Source`. `TranscriptSource` is a shared, schema-versioned record with `StreamId` (GUID), zero-based raw `RecordStartOffset`, exclusive `RecordEndOffset`, `PartIndex`, and `RecordSha256`. The runner stamps these during normalization, before event publication; HTTP/SSE mapping copies them unchanged. |
| `RunnerTranscriptDto` and `SessionRunnerTranscriptDto` | Optional `ProvenanceVersion` describing the entry format, plus the above fields on **each entry**. A snapshot can contain several producing generations and unknown history. There is deliberately no envelope generation to apply to its entries, and version presence alone authorizes no receipt. |
| `TranscriptEntry` / `AppDbContext` | Nullable `AcceptedStartedAt` (`timestamp with time zone`), `SourceKey` (bounded string, max 96) and `SourceJson` (`jsonb`). Derive `SourceKey` as `<stream-guid-N>:<record-start-decimal>:<part-index-decimal>` using invariant formatting. Add a unique filtered `(AgentSessionId, SourceKey)` index for non-null keys. Keep session-monotonic stored `Sequence` and native UUID/kind for presentation and legacy consumers. |
| S5 Docker lease journal and `TranscriptSidecar` | Journal retains a schema-versioned `TranscriptProvenance` history per bound native stream, with stream GUID, exact host path/native session/cwd binding, file identity, generation ranges and validated prefix checkpoints. The sidecar carries the journal stream reference for exact adoption; it is an index, not a second generation authority. The journal is outside guest mounts. |
| `SessionQueuedMessage` / `AgentTaskRefinement` | Nullable `DockerReceiptJson` on the queue and `ConfirmedReceiptJson` on the refinement, represented by new `server/Application/Dtos/DockerDeliveryReceipt.cs`. The receipt freezes session/queue identity, matched `TranscriptEntry.Id`, stored sequence, producing `AcceptedStartedAt`, source key/tuple, final wire SHA and referenced artifact hashes. Preserve `ConfirmedPromptSequence`/`ConfirmedAt` as convenient progress fields. The tuple, rather than those flags alone, is the audit/recovery evidence. |

**Where the generation becomes durable.** Extend S5's existing write-through launch journal, not `SaveSidecar`'s best-effort success path. Under the worker lease and session launch lock, before the new exec can write, record an open range `(AcceptedStartedAt, StartOffset, EndOffset=null)` for the exact stream. The range starts at the current raw file EOF. On a new empty stream it starts at zero; history already present without a journal is an explicit unknown prefix and can never be attributed to this launch. For a lazily created file, commit the stream ID, expected path and absence/start-zero state before exec, then bind its observed file identity durably before publishing a stamped event. Journal-write failure admits no child; a failure establishing the file identity publishes no attributed evidence and holds delivery.

A replacement generation may append to that same native conversation only after D-11 proves the previous exact container/writer stopped and its final transcript drain completed. Close the old range at the final EOF and commit the next open range in the launch-pending journal transaction before admitting the next writer. Do not use a quiet interval, a Docker client exit, the instant a server row changes, or a tailer's last poll as this boundary. At handover EOF must be at a complete JSONL boundary; partial final records or an unknown live writer retain an unresolved hold. No truncation, invented newline, file rotation or new native conversation is an automatic repair. Closing a range changes only its formerly open end; its generation/start and all prior closed ranges are immutable. Keep history when the current occupancy advances.

The journal stores non-overlapping ranges, the file identity and SHA-256/length of each validated prefix (including a closed generation's full prefix). Recheck the binding and recorded bytes on adoption/replay; short/replaced/rewritten files, malformed or overlapping ranges, unreadable journal, and conflicting identities yield unknown provenance and a visible hold, never a range synthesized from current settings. Extend the existing S5 journal validation and launch-pending commit barriers to include these members. No new age-based journal cleanup is introduced: retain stream histories with the dedicated worker's records, and unresolved obligations continue to protect the database transcript via D-16/G-149. Same-generation runner recreation reuses the open range; it does not create a generation boundary or reset its start.

**Where entries are stamped.** `SessionRunnerRuntime.StartTailerFor` and `RestoreTailerFromSidecar` pass the validated journal history to `GrokTranscriptTailer`. Track absolute byte offsets while holding partial lines; resolve a complete raw line to exactly one range before `Publish`. Persist the updated validated prefix checkpoint before publishing new attributed evidence. Set `AcceptedStartedAt` from that range, not from the newly constructed `RunnerSession`. Preserve the raw line's hash/source offsets and deterministic part ordinal. `GrokTranscriptNormalizer` must retain the original source tuple alongside buffered chunks; a later flush cannot change their producing generation. Reset/flush buffered state at a recorded generation boundary without synthesizing a successful TurnEnd. A receipt-bearing `UserPrompt` must come from a complete source record inside one known range; cross-boundary/unknown data remains unqualified. Transcript source metadata is outside the prompt text and is never inferred from a task marker supplied by the recipient.

Replay starts at zero as today, but resolves each row against the retained ranges. Thus an old raw prompt not yet ingested when G2 starts still carries G1; a new identical prompt after the committed boundary carries G2. A previously stamped G1 event delayed in HTTP delivery also stays G1. If the runner dies after the raw append but before normalization/checkpoint/publication, the durable open or closed range plus unchanged file bytes reconstruct the same attribution. If the journal/file cannot establish that fact, the outcome is held/unknown, not a guessed receipt. The native qualification must verify append/resume behavior and file identity on the pinned image; rewriting native history is a measured incompatibility to return to Investigate, not permission to weaken the range contract.

**Where the server preserves it.** `SessionRunnerHttpClient.MapTranscript` and `GetTranscriptAsync` preserve every entry's optional tuple for SSE and snapshot catch-up. `AgentSessionRuntime.PersistTranscriptAsync` validates tuple shape, normalizes its supplied token, and stores it unchanged; neither it nor individual-row retry/stub persistence may substitute the current session generation. For attributed Docker rows, deduplicate by the stable source key before assigning a stored sequence. A repeat of the same source key must agree on source hash, generation and normalized content; a conflict is recorded as an ingestion fault and does not overwrite/bless the original row. Replays do not create a second attributed row, including raw rows with no native UUID. Legacy null-source rows keep their existing UUID/kind or sequence dedup. An existing unknown row is not silently upgraded from the current generation. Unknown/stub/conflicting evidence cannot confirm Docker delivery.

Accept valid old-generation entries into durable history even when the session now runs G2; dropping them would lose the very late receipt D-16 requires. Separate persistence/receipt reconciliation from live runtime effects: old/unknown Docker entries must not set G2 working/idle state, drive its rules barrier or flush its composer as a fresh G2 TurnEnd. Apply generation/eligibility fences both to `ObserveTranscriptAsync`/`SyncTranscriptAsync` effects and to the database queries in `SessionMessageQueueService.IsWorkingBatchAsync` and `GrokRulesRefreshService`; skipping the callback alone is insufficient when delayed G1 rows receive high stored sequences. Preserve the explicit server-generated restart boundary for the accepted generation (`AgentSessionService` stamps the generation captured by the restart transaction); it has no native source tuple and can never be receipt evidence. Keep host queries unchanged. The current generation is needed for a new input decision, not for identifying the author of a historical row.

**Where recovery reads it.** Factor one Docker receipt-candidate predicate in `SessionMessageQueueService` and use it for inline confirm, grace confirm, `LateConfirmAttemptedMessagesAsync`, interrupted-attempt/Enter-only recovery and the refinement receipt link. First validate that the persisted attempt is bound; for a refinement, `queue.LastDeliveryGeneration` must equal the obligation's frozen accepted generation. Then the candidate must be a persisted, complete final-wire `UserPrompt` in the destination session, above the original persisted sequence/timestamp attempt floor, with valid non-null source provenance and `SessionGeneration.Equal(candidate.AcceptedStartedAt, queue.LastDeliveryGeneration)`. That last comparison is the single producing-generation guard PC-139 mutates; do not duplicate it in separate receipt matchers. Keep the floor and complete-text/hash checks independently necessary; an accepted generation alone is not receipt. `SourceKey` supplies identity, not a substitute for those checks. Never use current-generation equality as the historical receipt predicate and never allow null-equals-null. A never-attempted row cannot acquire a receipt merely because a matching prompt is present.

Return the matched row/tuple from the query, not only text/boolean. Commit `DockerReceiptJson` with the queue's Delivered/LateConfirmed verdict in the same transaction. `AgentTaskRefinementService.ReconcileAsync` copies that exact receipt idempotently into `ConfirmedReceiptJson`, verifies it against its frozen wire/binding/artifacts and sets progress. A cut before the verdict leaves both queue verdict and receipt absent; a cut after it but before the refinement link recovers the saved tuple. It may also late-confirm directly from newly ingested genuine source evidence when no verdict was saved. No path marks Confirmed from Sent alone, an old verdict lacking provenance, or a newly looked-up current generation. Unresolved or conflicting evidence remains retained and visible.

For example, an attempted G1 refinement can confirm from a delayed genuine G1 row after G2 starts, with zero further input. A byte-identical G2 row, a null-generation row or an old full prompt below the floor cannot confirm it. Without genuine G1 evidence the obligation stays held; replacing the generation never authorizes retyping it into G2. This also applies to the Docker destination variants of completion receipts; their existing obligation identity is retained. Under-ceiling row-less Docker Now captures the expected generation before input for its immediate check, but gains no durable recovery promise.
...
VD-1 expands S4 in S4a..S4c, with S4d added by the provenance amendment. S4a precedes S4b. S4d extends the S3 contracts/S4 tailer/S5 launch journal and must be in place before S4c receipt recovery can pass. All four are required before S6 instruction-delivery qualification.
...
| S4d: durable transcript generation and receipt provenance | Extend new S5 `src/Antiphon.SessionRunner/DockerWorkerLeaseStore.cs` and lifecycle coordinator with retained generation ranges; `src/Antiphon.SessionRunner/{SessionRunnerRuntime,GrokTranscriptTailer,GrokTranscriptNormalizer,TranscriptSidecar}.cs`; `src/Antiphon.SessionRunner.Contracts/SessionRunnerContracts.cs`; `server/Application/Dtos/SessionRunnerDtos.cs` and new `DockerDeliveryReceipt.cs`; `server/Infrastructure/Agents/SessionRunner/SessionRunnerHttpClient.cs`; `server/Application/Services/{AgentSessionRuntime,AgentSessionService,SessionMessageQueueService,AgentTaskRefinementService,GrokRulesRefreshService}.cs`; `server/Domain/Entities/{TranscriptEntry,SessionQueuedMessage,AgentTaskRefinement}.cs`; `AppDbContext.cs` and CLI-generated `server/Migrations/*_AddDockerTranscriptProvenance*` plus snapshot. Implement D-18 fields, source-key dedup, generation-aware matching shared by inline/grace/late-confirm, live-effect fences and durable receipt linking. | Extend `DockerGrokTranscriptTests.cs`, `DockerGrokRecoveryTests.cs`, `DockerGrokRunnerWireTests.cs`, `DockerGrokDeliveryRecoveryTests.cs` and `DockerGrokTestWorld`: actual raw ACP append -> production tailer -> HTTP event/snapshot mapping -> real ingestion -> queue/refinement receipt; no fixture-assigned generation. Required cases and existing guard bindings appear in the final verification amendment. Regression: `GrokTranscriptTailerTests`, `SessionRunnerGenerationWireTests`, `SessionGenerationDeliveryOverlapTests` and existing R-7/R-8. |
...
The **historical first-amendment subtotal** is 148 guards / 148 unique PC mappings (original P=15, U=45, I=80; then I=88). The focused TestDesign review below expanded that inventory to 172; D-20 adopts its additional controls and floors. The original 140 recipes and baseline floors are preserved. This paragraph records task `0d678e51`'s handoff, not the current inventory. No test execution, mutation run or live delivery success was claimed there. Its next stage was test-design.
...
**Historical disposition at `3cfe11ae`: return to Plan; VD-1 was not Code-ready.** The following findings and recipes are retained as review evidence. D-18/D-19 and the final Plan resolution below answer the two findings; the required focused TestDesign re-audit has not yet occurred. At this review, the outbox, keyed queue and common file-store decisions provided implementable seams for PC-141..148, but left two affected original controls open:
...
**Historical TestDesign handoff audit (`6b037196`):** inspected bodies and nearest fixtures are listed; inventory guards=172, mapped=172, missing mappings=0, duplicate PC mappings=0. PC-141..172 have concrete mutation, exact method, decisive assertion and specified setup contracts; they were not implemented or executed. The all-PC-executable Code gate was rejected because PC-81's stated assertion could survive and PC-139 lacked a production provenance seam. That report's next stage was Plan. The resolution below is the current handoff; it does not retroactively turn that rejected review into an approval.

### Plan resolution: generation provenance, PC-81 and adopted inventory

Plan task `5e3ba4d4` resolves VD1-TD-1 with D-18/S4d and VD1-TD-2 with D-19. These are selected designs, not provisional operator defaults. No operator decision is outstanding. The required next step is a **focused TestDesign re-audit**, not Code: verify that the new producer/ingestion/receipt seam and its guard arrangements are executable. The old statements above that no transcript schema was selected or that these cases were rejected describe the prior review, not the amended design.

Inspection at `3cfe11ae` covered transcript entity/DTO/sidecar definitions; generation normalization; Grok tailer read, partial-line, normalize, publish and snapshot paths; normalizer source/flush behavior; runner `StartTailerFor`, `RestoreTailerFromSidecar` and `SaveSidecar`; HTTP event/snapshot mapping; runtime `ObserveTranscriptAsync`, `CatchUpTranscriptAsync`, `SyncTranscriptAsync`, `PersistTranscriptAsync` and individual-row persistence; and queue late-confirm/sequence-floor/timestamp-floor matchers. This independently confirms the missing field, byte-zero replay and ingestion seams reported by TestDesign. Existing fixture/body inspection in the prior review remains inherited evidence; this amendment ran no tests, build, migration, Docker operation or provider turn.

**PC-81 approved executable addendum (original row remains verbatim).** Keep the production mutation `Use yyyyMMddHHmmss alone for refinement filename` and the exact method `DockerGrokBriefPointerTests.Same_second_refinements_do_not_overwrite`. Submit two different full renderings through real `RefineAsync` in the same frozen second, with a valid same-generation recipient and otherwise valid artifacts/rules. Let A publish/deliver, then accept B; catch only the documented accepted-but-held conflict if B materialization refuses, recover its accepted GUID from the persisted obligation, and run a bounded number of real scanner/flush passes to completion. Do not wait until B is delivered as the mechanism for failing the test. The decisive checks are:

```csharp
firstGuestRead.Bytes.ShouldBe(firstBodyBytes); // retained original assertion
recipient.CompletePromptsFor(firstRefinementId).Count.ShouldBe(1);
recipient.CompletePromptsFor(secondRefinementId).Count.ShouldBe(1); // added decisive red
secondGuestRead.Bytes.ShouldBe(secondBodyBytes);
```

Assert prompt counts before dereferencing the possibly absent second read, so the mutant fails the explicit count assertion, not a null/fixture exception. The fixed method also checks distinct persisted artifact paths, A's bytes after B and after service recreation, and each receipt's queue/source identity. The timestamp-only mutant collides at the file store, keeps A correct and leaves accepted B held; zero complete prompts for B makes the added assertion red. Do not mutate immutable-file validation to force overwrite. Keep the identical-rendering/distinct-ID companion and the same-ID retry companion from VD-1; they respectively prove two accepted instructions and one replayed instruction, using their individual attempt floors/receipts rather than content-only counts.

**Provenance acceptance and affected-control arrangements.** Extend the existing Docker test world with a raw ACP recipient: it appends native-format rows only after the actual queue/adapter submission. The production launch/journal boundary establishes its generation; the fixture never supplies `AcceptedStartedAt` on an expected successful transcript. Use scratch native files and the production Grok tailer, serialize through the actual runner DTO and client mapping, then call real server ingestion. A delayed HTTP envelope may be held by the fixture only after production stamping. For delayed raw ingestion, stop/recreate the tailer with only file/journal state retained. Provider recreation keeps only the isolated DB, owned files/journal and actual recipient submission history.

| Required method/variant and owning file | Observable result / guard binding |
|---|---|
| `tests/Antiphon.SessionRunner.Tests/DockerGrokTranscriptTests.cs`: `Generation_ranges_survive_retail` | Open G1 through real journal preparation, append G1 input/ACP, close at the real writer boundary, open G2, append byte-identical G2 text; replay from zero. Each normalized entry has its original generation/source key. Include delayed normalization, partial lines within one generation, buffered normalizer flush, same-generation runner restart, preexisting unknown prefix and missing native UUID. D-18 supplies the source for G-139's end-to-end variants. |
| `tests/Antiphon.SessionRunner.Tests/DockerGrokRecoveryTests.cs`: variants of `Lost_or_corrupt_journal_holds`, `Spawn_crash_does_not_double_exec`, and the existing launch-pending/drain methods | Cut before/after provenance journal commit and before/after raw append/checkpoint. Uncommitted range creates no child; committed range can be recovered without relabeling; missing/corrupt/overlapping history, replaced/short/rewritten file and incomplete final drain hold. G-83/G-89/G-97/G-100/G-102/G-136 retain their original recipes; TestDesign must check which new provenance validation arms need distinct PCs. |
| `tests/Antiphon.Tests/Agents/DockerGrokRunnerWireTests.cs`: `Transcript_provenance_roundtrips_stream_and_snapshot` | Real serializer and `SessionRunnerHttpClient` preserve G1/G2/null entries in one snapshot and in SSE, with source offsets/hash and microsecond token. Envelope version never fills a missing entry token. Null legacy fields remain legal. No fake client that returns a pre-labeled server DTO can satisfy this boundary. |
| `tests/Antiphon.Tests/Application/DockerGrokDeliveryRecoveryTests.cs`: `Transcript_provenance_survives_ingestion_restart` | Ingest production-stamped G1 after the session row becomes G2, recreate provider/context, and assert persisted G1 token/source. Replay is one row even without UUID; same text at distinct source offsets is two rows. A conflicting duplicate retains original evidence and records a fault; no current-generation upgrade. CLI migration round-trips legacy nulls and the new receipt/source JSON. |
| Q `Receipt_requires_bound_generation` / PC-139 | First arrange no genuine G1 receipt, then let G2 actually receive the identical full wire through a separate authorized input while the original attempt and obligation remain G1. Ingest via the production chain. All binding/file/floor/text prerequisites are valid, so only the **candidate-to-attempt** generation comparison rejects it: `receipt.Accepted.ShouldBeFalse()`. Mutate that comparison alone, not the earlier attempt-to-obligation binding check. Test inline, grace and late paths through the same predicate; null/unknown provenance also rejects. Observe the receipt decision before any unrelated downstream guard can mask red. |
| Q `Accepted_prompt_is_not_retyped` / PC-113; positive companion of `Refinement_cannot_retarget_new_generation` / PC-145 | G1 really receives the prompt, then delay (a) the stamped event or (b) raw normalization. Advance to G2 only through the production writer boundary and recreate runner/server as applicable. Deliver original evidence and reconcile to a G1 receipt; total original submit count is one and replacement-generation recovery submits are zero. Also cut verdict/receipt transaction and the later refinement link separately. PC-145's negative remains never-attempted with no old receipt, so removing the pre-input gate can still produce its stated red. |
| Q `Complete_prompt_is_required` / PC-109 and `Receipt_floor_is_required` / PC-110 | Hold generation/source/file prerequisites valid while exercising incomplete text and below-floor full text. Preserve each original mutation/assertion independently; added provenance must not become a fixture fault or mask the original red. No generation label can join prompt fragments or make a stored pointer receipt prove a guest read. |
| Q `Old_generation_replay_does_not_release_current_delivery`; R-8 Docker completion variants | Busy G2 remains busy with zero new queue input when G1 TurnEnd/ack is replayed; old valid prompt remains usable only for its original receipt. Completion delivery freezes the same matched tuple as the queue without changing completion identity. Cover late receipt before task settlement and after terminal settlement, with no new automatic submit. |

Q above is `tests/Antiphon.Tests/Application/DockerGrokDeliveryRecoveryTests.cs`. Each new method is a concrete implementation/ordinary-verification contract; the names are not claims of existing tests. Use the preserved exact-method `Invoke-C575Selection` recipe with a fresh TRX. Require the nonzero executed cases and both old-event and old-raw recovery paths. Add `SessionRunnerGenerationWireTests/*` and `SessionGenerationDeliveryOverlapTests/*` to affected ordinary regression coverage; preserve R-4/R-7/R-8/R-10 and host null-generation behavior. Live/native qualification additionally verifies the journal boundaries against actual pinned Grok append/resume semantics; scratch files prove the logic only.

**Adoption of all 24 additional controls.** D-20 makes the preceding G/PC-149..172 inventory and its exact methods, mutations, assertions, fixture cuts and unmasking arrangements required implementation work. They are not suggestions or a second alternative inventory. Slice ownership is:

| Slice | Additional controls owned | Required effects |
|---|---|---|
| S4a | G/PC-154..156, 160, 164..169, 171..172 (12) | Atomic/authorized/attempt-safe cancellation; keyed amend refusal; acceptance order/state checks; separate existing-key ID/destination/hash checks; database uniqueness; lossless input; established binding. |
| S4b | G/PC-157..159, 161..163, 170 (7) | Snapshot-before-I/O, original bytes/members on replay, frozen amend refusal, final pointer ceiling, completion agreement and atomic file publication. |
| S4c | G/PC-149..153 (5) | Transcript/task/session retention, terminal dedup tombstones after queue pruning, and terminal-task no-submit. G/PC-147 remains queue pruning only. |

The corresponding files/tests are S4a..S4c's named files plus the exact B/Q methods already specified in PC-149..172; S4d adds the receipt fields those tests must preserve. Keep every prior arrangement, especially production pre-delete observations for FK-masked retention controls, distinct providers for uniqueness races, and a clock that actually wakes the hosted scanner. Do not infer that one retention test covers all passes or that another guard's rejection is the intended assertion red.

**Authoritative adopted accounting.** Canonical mappings are G/PC-1..140 once, G/PC-141..148 once (their repeated review rows are clarifications), and G/PC-149..172 once. There are **172 adopted guard/control mappings: P=15, U=45, I=112**, no missing numbers or duplicate canonical mapping. The earlier 148/183/444.3 figures are historical subtotals, not dispatch budgets.

| Adopted scope | Code floor (minutes) | Mutation floor (minutes) |
|---|---:|---:|
| Original 140, unchanged | 174 | 413.5 |
| First eight VD-1 controls and expanded cases/R-10 | +9 | +30.8 |
| TestDesign's 24 added controls | +20.4 | +92.4 |
| **Adopted minimum** | **203.4** | **536.7** |

Combined adopted floor is **740.1 minutes (12 h 20 m 6 s)**, with zero credited savings. These are floors, not measured or final estimates for D-18's additional source/ingestion scenarios. The four newly named ordinary methods above (`Generation_ranges_survive_retail`, `Transcript_provenance_roundtrips_stream_and_snapshot`, `Transcript_provenance_survives_ingestion_restart`, `Old_generation_replay_does_not_release_current_delivery`) add a provisional ordinary allowance of **4 x 0.85 = 3.4 minutes**, giving a forward Code floor of **206.8 minutes** before further parameter-matrix measurement. Use the I integration rate for these cross-boundary fixtures even though some run in the runner project. Original 174/413.5 and the adopted 203.4/536.7 arithmetic remain unchanged and visible.
