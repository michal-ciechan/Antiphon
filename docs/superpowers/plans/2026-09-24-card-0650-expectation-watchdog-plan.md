...
| **V-2**, 15 executions | `ExpectationPipelineTests`: `C650_Queue_age_and_dispatch_progress_use_exact_boundaries`, `C650_Requeue_resets_age_but_Held_does_not`, `C650_All_paths_fenced_is_immediate`, `C650_Partial_or_unknown_path_is_not_global_fence`, `C650_Quota_fences_new_admission_only`, `C650_Capacity_deficit_requires_continuous_ready_work`, `C650_Ordinary_waits_are_not_global_fences`, `C650_Stale_land_or_cap_hold_beside_unrelated_running_work_opens_the_episode` (R3), `C650_Unknown_observation_does_not_resolve`. `ExpectationSnapshotTests`: `C650_Scope_excludes_other_boards_and_ambiguous_callers`, `C650_Counts_runner_lanes_and_excludes_specialists`, `C650_Backlog_candidates_exclude_owned_held_unrated_and_open_work`, `C650_Hold_classifier_matches_dispatcher_messages`, `C650_Archived_board_is_inactive_for_detection`, `C650_Persisted_lands_and_cap_occupants_decide_the_stalled_pipeline_episode` (R3 repair 2: isolated-DB `ReadLandsAsync`/`ReadCapOccupantsAsync`; stale, held or finished land and a short or ended cap open the episode, a moving land and a full live cap stay quiet). | At N-1 tick no condition, at N condition. A dispatch on another board is not progress. Local healthy/server2 down negative; common repository fence positive. Fresh all-candidate quota vs missing/stale sample. Known land/cap/pin/remote preparation negative. |
...
| CP-4 | S2 | `tests/Antiphon.Tests -> bin-c650-r2/` | pipeline-green | `/*/*/(ExpectationPipelineTests*)\|(ExpectationSnapshotTests*)\|(DispatchHeldAttentionTests*)/*` | V-2, R-1 | all 15 new + 8 existing, 0 failed/skipped | 23 | 1.9 |
