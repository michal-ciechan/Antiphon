...
Status: Implemented on `feat/card-task-76079973` through `649f95fe` (missing-worktree admission skip). Round-3 repair (task bac35aa3, docs/tests-only): the skip is behaviorally complete; this round documents D-4/D-6/Risks, records V-2b/V-15/V-16, adds V-17/V-18 and PC-9/PC-10. Ordinary Review is next; PCs stay pending for SourceLanding Mutation. Original Code task `76079973` remains the landing owner.
...
- **Round-3 repair (task bac35aa3):** `ProbeCheckoutIndexLockAsync` returns null when `!Directory.Exists(checkout)`, checked both before and after `InspectIndexLockAsync`. Cleanup never clears `AgentTask.WorktreePath`; probing a gone directory mapped `InspectIndexLockAsync`'s `index_lock_path_error` through `Evaluate` into a permanent `git_index_lock_held` hold (no attempt consumed). A checkout whose directory still exists but whose `.git` file/path is deleted or unreadable remains Held. `InspectIndexLockAsync` itself is unchanged: a missing checkout still returns `Reason=index_lock_path_error` and does not throw.
...
`server/Application/Services/GitIndexLock.cs` (static): `Observe(path, now, census)`, `Classify(observation, staleAfter)`, `FormatDetail(...)`, `Evaluate(observation, staleAfter, now)`. There is no reclaim helper. `ILandingGit.InspectIndexLockAsync(string checkout, CancellationToken)` returns `LandingIndexLockObservation(string Path, bool Present, DateTime? LastWriteUtc, long? Length, IReadOnlyList<(int Pid, DateTime? StartUtc)> CandidateHolders, string? Reason)`; `LandingGit` implements it with `rev-parse --path-format=absolute --git-path index.lock` plus `FileInfo` plus the census. `FixtureGit` inherits it, so land tests exercise the real implementation on scratch repositories. A probe failure returns `Reason` (`index_lock_path_error`) and does not throw. `Evaluate` maps any `Reason` to `Held` (safe direction) rather than proceeding blind — that mapping is still correct for an existing but unreadable checkout.

**Round-3 repair (task bac35aa3):** admission does **not** treat a missing checkout directory as held. `ProbeCheckoutIndexLockAsync` skips the hold when `!Directory.Exists(checkout)` (before and after Inspect). `InspectIndexLockAsync` still returns `PathErrorReason` for a gone path (V-2b). The distinction is: directory gone → no hold (V-15/V-16); directory exists, `.git` deleted/broken → `git_index_lock_held` with `index_lock_path_error` in the detail (V-18).
...
| V-2 | `GitIndexLockPathTests.Path_resolves_main_and_linked_worktree` on `LandingGitFixture` | `InspectIndexLockAsync(Repository).Path` ends with `\.git\index.lock`; `InspectIndexLockAsync(Source).Path` contains `\worktrees\` and ends with `\index.lock`; both `Present == false` before creation and `true` after. |
| V-2b | `GitIndexLockPathTests.Missing_checkout_returns_path_error` | `Reason == index_lock_path_error`, `Present == false`. Inspect does not throw. |
...
| V-15 | `AgentTaskLandIndexLockTests.Missing_source_worktree_does_not_hold_as_index_lock`: `worktree remove --force` the source, then `RunAsync()` | not `LandRunResult.Held`; `HoldReasonCode` is neither lock code; no Held event; any `LandRefused` detail does not contain `git_index_lock_held` or `index_lock`. |
| V-16 | `AgentTaskLandIndexLockTests.Reland_after_removed_worktree_is_not_held_as_index_lock`: land once, remove the source worktree, leave `AgentTask.WorktreePath` set, re-POST | not Held; no Held event whose detail contains `git_index_lock_held` or `index_lock_path_error`. |
| V-17 | `GitIndexLockTests.Evaluate_path_error_is_held` (pure): `LandingIndexLockObservation` with `Reason=index_lock_path_error` and a path | `Kind.Held`; detail contains the path and `index_lock_path_error`. |
| V-18 | `AgentTaskLandIndexLockTests.Existing_source_worktree_with_broken_git_holds_as_index_lock`: source directory still exists; delete the worktree `.git` file; `RequestAsync(expectedSourceSha)` then `RunQueuedAsync()` (explicit SHA so the harness does not `rev-parse` the broken worktree) | `LandRunResult.Held`; `HoldReasonCode == git_index_lock_held`; `HoldDetail` contains `index_lock_path_error`; `Attempt == 0`; no operation. |
...
| PC-9 | Remove both `Directory.Exists` guards in `ProbeCheckoutIndexLockAsync` | V-15 and V-16 go red (`git_index_lock_held` / `index_lock_path_error`). V-18 stays Held. |
| PC-10 | In `Evaluate`, skip the `Reason` branch (fall through to `Classify`) | V-17 goes red (`Kind.None` when `Present` is false). V-18 no longer holds. |
...
- `LandingGit.InspectIndexLockAsync` must not throw for a missing checkout; return `Reason`. `Evaluate` maps any `Reason` to `Held`. Admission (`ProbeCheckoutIndexLockAsync`) skips the hold when the checkout directory is gone (checked before and after Inspect) so a cleaned-up worktree is not held as `git_index_lock_held`. An existing checkout with a broken `.git` still holds (V-18).
...
--- next stage ---
next: review
handoff: Round-3 docs/tests-only: D-4/D-6/Risks now match ProbeCheckoutIndexLockAsync Directory.Exists skip; V-2b/V-15/V-16 recorded; V-17 Evaluate path_error Held and V-18 exists-but-broken-.git hold added; PC-9/PC-10 pending Mutation. Original Code 76079973 lands after ordinary Review. restart: none
