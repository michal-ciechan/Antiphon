...
| V-18 | `AgentTaskLandIndexLockTests.Existing_source_worktree_with_broken_git_holds_as_index_lock`: source directory still exists; delete the worktree `.git` file; `RunAsync()` | `LandRunResult.Held`; `HoldReasonCode == git_index_lock_held`; `HoldDetail` contains `index_lock_path_error`; `Attempt == 0`; no operation. |
