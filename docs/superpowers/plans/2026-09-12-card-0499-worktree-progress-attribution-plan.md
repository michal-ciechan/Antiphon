...
- V-6: source landing in flight holds | Dispatch (`AgentTaskDispatchBaseGuardTests`, real repo, owner with `LandRequestedAt` set) | `AgentTaskDispatchBaseGuardTests.C499_V06_ARepairIsHeldWhileItsOwnerIsLanding` | repair is seeded on a **different card** (or `CardId == null`) from the owner so CARD-0215 `EvaluateCardSiblingBaseAsync` cannot match; first tick: `Status == Queued`, `WorktreePath == null`, one Held event whose Detail is exactly `{owner short} is landing` (no `held:` prefix, no `kept branch`); clear `LandRequestedAt`; second tick: `Dispatched`, Held count stays 1, baseline persisted.
- V-7: baseline lives inside the claim; crash recovery | Dispatch (`RepairSourceWorld` with the `LandingSafetyHarness.SaveFault` interceptor registered on the DbContext, `AfterCommit=false`, faulting the claim's commit) | `C499_V07_ABaselineIsPersistedOnlyWithTheClaimAndRecapturedOnRetry` | faulted tick throws `InjectedSaveFailure`; leftover worktree directory exists at `OwnerSha` (CreateForTaskAsync ran inside the claim); row `Status == Queued`, `ProgressBaselineJson == null`, `DispatchedAt == null` (the interceptor aborts every `SaveChangesAsync`, so a null JSON column cannot independently fail); second tick (fault cleared): `Dispatched`, `WorktreePath` equals the leftover path, `ProgressBaselineJson.primary.localSha == rev-parse HEAD` of the adopted tree `== OwnerSha`, one Dispatched event.
...
- PC-6: break G-6 by changing `if (owner?.LandRequestedAt is not null)` to `if (owner is null)` in `AgentTaskDispatcher` so the repair-owner landing hold never fires; expect `AgentTaskDispatchBaseGuardTests.C499_V06_ARepairIsHeldWhileItsOwnerIsLanding` red at `held.Status.ShouldBe(AgentTaskStatus.Queued)` or `heldEvents[0].Detail.ShouldBe("{short} is landing")`. V-06 seeds the repair on a different card from the owner so CARD-0215 sibling-base hold (`UnlandedSibling.HoldDetail` containing `kept branch`) cannot satisfy the test.
- PC-7: break G-7 by capturing and assigning `ProgressBaselineJson` **before** `CreateForTaskAsync` (the original "own `SaveChangesAsync` before the `FOR UPDATE` claim re-read" is not realizable: `DispatchSaveFault` throws on every save, capture requires `WorktreePath`, and the claim row is locked). Expect `C499_V07_ABaselineIsPersistedOnlyWithTheClaimAndRecapturedOnRetry` red at `leftover.ShouldNotBeNull()` (worktree never created). `ProgressBaselineJson.ShouldBeNull()` cannot fail under the interceptor.
...
- PC-35: break G-35 by giving each `PinBaselineAsync` call a unique ref (`baseline-{name}` → `baseline-{name}-{guid}` at `TaskProgressGit.PinBaselineAsync`); expect `C499_V30_ProbePinsAreBoundedAcrossRetries` red at `ProgressPins().Count.ShouldBeLessThanOrEqualTo(4)`. The observation-pin mutation at `ObserveExactRefAsync` (`refs/antiphon/progress/<task:N>/observe-…`) is inert in V-30: the claimed commit is made in the owner tree and pushed, so it is already local and `cat-file -e` never takes the fetch/observation-pin path.
