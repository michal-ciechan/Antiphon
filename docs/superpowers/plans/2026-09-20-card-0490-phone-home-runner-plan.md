...
| PC-12 | Enqueue launch before awaiting the binding transaction commit. T `PhoneHomeStandingLaunchTests.Launch_handoff_cuts_preserve_owner_and_reservation`: gate commit/force rollback, observe from independent connection, `commandsBeforeCommit.ShouldBeEmpty()` fails. |
