...
| PC-136 | ignore Enabled=false | `E.Hosted_timer_disabled_polls_nothing_across_ticks` (Code round 2 remap from `A.Disabled_sweep_polls_nothing`, which calls SweepAsync manually) | production-registered hosted service on real HTTP/DB: runner follow GET count stays zero across three fake-clock hour ticks and pins/watermark unchanged. |
