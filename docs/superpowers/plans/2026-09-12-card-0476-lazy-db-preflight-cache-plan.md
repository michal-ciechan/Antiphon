...
- V-8: clone path on a faulted lifecycle | Unit | `Clone_request_on_a_faulted_lifecycle_does_not_drop` | `CreateIsolatedSchemaAsync` throws the bootstrap fault as a bare `InvalidOperationException` (not an `AggregateException` wrapper); `Drop == 0`; `Create == 1`.
- V-8b: drop path on a faulted undisposed lifecycle | Unit | `Drop_on_a_faulted_lifecycle_does_not_drop` | after the throwing `EnsureReadyAsync`, `DropClonedDatabaseAsync("test_x")` throws `ObjectDisposedException` naming `TestDbFixture`; `Drop == 0`.
...
- V-11: DB-free selection starts nothing | Integration, fresh child | `A_db_free_exact_method_selection_constructs_and_starts_no_database` launches `Child_db_free_selection_touches_no_database` with `ANTIPHON_PTY_BACKEND=modern` in the child's extraEnv; the child constructs `new TestDbFixture()`, builds explicit options, asserts the guards (`SessionRunner__BaseUrl == http://127.0.0.1:1`, `Delegation__CheckInterpreterEnabled == false`, `ANTIPHON_PTY_BACKEND` null — which can only hold if `PtyBackendEnvGuard` cleared the exported value) and `TestDbFixture.Lifecycle.IsRequested == false` | child exit 0; `child.trx` has exactly one executed result, named `Child_db_free_selection_touches_no_database`, outcome Passed; `lifecycle.json` has `state == "never-requested"`, `create == 0`, `start == 0`, `teardownDispose == 0`, `runnerBaseUrl == "http://127.0.0.1:1"`, `ptyBackend == null`.
- V-11b: recursion guard | Integration, fresh child | `A_class_filtered_child_skips_every_parent_probe`: launch with the class filter and the marker | child exit 0; TRX shows every parent-probe method Skipped or NotExecuted only (Passed is not accepted); child methods may Pass or Skip; sibling `probe-*` count under `Path.GetDirectoryName(run.Root)` is unchanged across the child (exactly the parent's one new root); `lifecycle.json` `create == 0`.
- V-11c: depth cap | Integration, fresh child | `A_child_at_depth_two_refuses_before_any_work`: `bypassLaunchDepthCap: true` so the parent actually starts a child with `depth=2` (the launch-side cap would otherwise refuse `Process.Start`); exact filter on `Child_db_free_selection_touches_no_database` | child exit nonzero; TRX/stderr contains `depth`; no `lifecycle.json` counters above 0. The launch-side cap (inherited depth + 1, refuse `Process.Start` when that exceeds 1) is the bound that keeps a skipped-from parent from starting a grandchild.
...
- G-19: D-3 `ProductionRunnerGuard` and `PtyBackendEnvGuard` remain eager and independent of the DB lifecycle | PC-19, PC-19b
...
- PC-7: break G-7 by reading `ConnectionString` unconditionally at the top of `CreateDbContextOptions(string?)`; expect `Child_db_free_selection_touches_no_database` / `A_db_free_exact_method_selection_constructs_and_starts_no_database` red at `TestDbFixture.Lifecycle.IsRequested.ShouldBeFalse()` (the explicit-options path must not request the lifecycle).
...
- PC-19b: break the `PtyBackendEnvGuard` half of G-19 by skipping `ClearInheritedPtyBackend` when `TestDbFixture.Lifecycle.IsRequested` is false; expect `A_db_free_exact_method_selection_constructs_and_starts_no_database` red at `ptyBackend` null (the db-free child is launched with `ANTIPHON_PTY_BACKEND=modern`, so null can only hold if the guard cleared it).
...
- PC-21: break G-21 by making `DropClonedDatabaseAsync` call `EnsureReadyAsync()` instead of reading the ready state; expect `Drop_on_a_faulted_lifecycle_does_not_drop` red at `ObjectDisposedException` (a faulted undisposed lifecycle has no ready state; `EnsureReadyAsync` rethrows the bootstrap fault instead of `ObjectDisposedException` naming `TestDbFixture`).
...
- PC-55: break G-55 by deleting the inherited-marker skip from ONE parent probe method (bounded variant); expect `A_class_filtered_child_skips_every_parent_probe` red at the sibling `probe-*` count (`before + 1` across the child). The launcher reads inherited depth from `ANTIPHON_C476_PROBE` (absent → 1), serializes inherited+1, creates the sibling `probe-*` root, then refuses `Process.Start` when that depth exceeds 1, so the skipped-from parent creates a sibling root but starts no grandchild.
