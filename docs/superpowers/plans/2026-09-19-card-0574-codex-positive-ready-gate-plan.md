...
*Plan resolution (task 33080007): narrowed to the model/effort/cwd family by D-8 after a 574-archive sweep found no empty-composer instance of the context/shortcut footer; see the Plan revision section, Seam 1. R-6 stands; the derived negative fixtures are listed in the regression plan's "Context/shortcut footer" row.*
...
*Plan resolution (task 33080007): the fence is in S3's production scope by D-9 (capture the normalized `StartedAt` at entry, pass it in the attached-adapter catch); R-61/G-59/PC-59 stand unchanged; the existing interrupted-resume ready-false method moves to `["KillGeneration", "Dispose"]`. See the Plan revision section, Seam 2.*
...
*Plan resolution (task 33080007): one shared Running guard in `DeliverNextLockedAsync`, redundant per-entry checks removed, `SendNowAsync` refuses like `Mode.Now`, `AgentSessionRuntime.SendInputAsync` exempt with reason (D-10). G-42/G-43/G-44 consolidate to one guard and one control that reddens R-42/R-43/R-44; a new guard covers the `SendNowAsync` refusal. See the Plan revision section, Seam 3.*
...
- Additional context/shortcut-only positive footer family and measured narrow/wrapped layouts: no complete positive capture in the inspected evidence. R-6 rejects them for now; Plan must resolve contract item 3 before Code. *Resolved by D-8: no such positive family exists in 574 archives; support stays narrowed.*
- Production repair of resumed-launch generation fencing is outside this documentation stage. Plan must resolve its scope because the current optional-null cleanup contradicts the stated D-7 assumption. The required test and PC remain inventoried, not waived. *Resolved by D-9: the fence is in S3's production scope.*
- Production repair of explicit-flush/turn-end Starting admission is likewise a Plan scope decision. Both negative ordinary cases remain required; settle the actual guard topology before Code and Mutation. *Resolved by D-10: one shared helper guard plus manual refusal, in S3's production scope; TestDesign consolidates the mapping.*
...
Plan revision validation (task 33080007): the verification design above is annotated with three italic resolution notes and three out-of-scope annotations only; no R/G/PC row was renumbered or removed. Source lines cited were read in `1a0994ec`. The archive sweep touched only `C:\logs\antiphon\session-runner\*.ansi.log` read-only and its record was appended to the captures artifact. No build, test or CLI launch was performed; `git diff --check` was run before commit.
