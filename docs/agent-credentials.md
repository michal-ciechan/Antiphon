...
own login. `deploy-parent` always leaves the host file present at 0600 (an absent bind source would
become a directory) and records only whether it is non-empty. A locked vault or a missing item is a
warning, not a failed deploy: a previously delivered file is left as it was, and with none the file
is empty and the runner reports Claude signed out (`claudeAuth=logged-out`). A launch env cannot carry it: the server refuses
