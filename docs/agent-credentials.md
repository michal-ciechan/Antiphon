...
|---|---|---|---|
| **Primary:** `CLAUDE_CODE_OAUTH_TOKEN`, a `claude setup-token` OAuth token for the operator's Max subscription | Generated once on the desktop with `claude setup-token`; stored in the vault item `antiphon/server2/claude-oauth-token` (login password) | The vault item, copied onto server2 only when a deploy asks. By default `deploy-parent` leaves `/home/mc/antiphon-server2/secrets/claude_oauth_token` unchanged and does not need `BW_SESSION`. `-RefreshClaudeToken` on `scripts/verify-docker-stack.ps1 -Case deploy-parent` (or manifest `refreshClaudeToken`: true, or `ANTIPHON_REFRESH_CLAUDE_TOKEN=1`) reads the relay session (`BW_SESSION` or `~/.bw-session`) and streams the item over SSH **stdin** to that file, 0600, owner `mc` | `docker-compose.server2-runner.yml` mounts that file read-only at `/run/antiphon/claude-oauth-token` (never an `environment:` entry, which `docker inspect` would print); `dind-entrypoint.sh` exports `CLAUDE_CODE_OAUTH_TOKEN` from it after dockerd starts and just before the runner, so the runner process and every pty child inherit it |
...
The token is never written to the repository, the image, an env file, a compose `environment:`
entry, a desktop file, an SSH argument, a card or a log, and it is never copied from the desktop's
own login. `deploy-parent` does not contact the vault unless a refresh was requested, so a deploy does not need `BW_SESSION`. The default prints `Claude token: keeping the existing server2 file (no vault refresh requested)` and leaves the server2 file untouched. The host lane still creates the file at 0600 when it is missing (an absent bind source would
become a directory) and records only whether it is non-empty in `claude-oauth-token-present.txt`. A missing or empty file warns that the runner will report `claudeAuth=logged-out` and names `-RefreshClaudeToken`. A requested refresh whose vault is locked, or whose item cannot be read, warns `refresh skipped, previous file unchanged` and does not replace the file. A launch env cannot carry it: the server refuses
