...
- **A queue retry must belong to the composer that took the typing (CARD-0501).**
  ...
  remains uncharged when the body write never left (even if an overlay Esc preceded it): once the
  body or an earlier Enter left, the loss keeps the attempt and its floor so the next flush late-confirms
...
- **Capacity auto-resume is paced and transcript-confirmed (CARD-0412).** Supervisor reconciliation is the sole granter (one outstanding grant per execution kind, oldest BlockedAt then Id for fresh waits; revoked handoffs rejoin at their re-arm DueAt so dead consumers cannot monopolize FIFO). Queue, standing start and dispatcher redeem that grant at the execution boundary with 60s spacing. An unredeemed grant expires after two configured admission intervals measured from `GrantedAt`; reconciliation re-arms its wait with DueAt at least two admission intervals in the future and grants the next eligible consumer. Wave revocation uses the same re-arm instead of leaving a Reheld dead end. Expiry spends no admission: only redemption increments AdmissionCount, and unsuccessful admissions still reach Exhausted. Re-arming a new action updates queue redemption keys atomically, while the session launch-dedupe stamp stays on the last launched action so the supervisor can start the next attempt; receipt-backed revalidation retains its action identity and admission count. This also bounds receipt-backed revalidation when its consumer has disappeared. Stalled-admission re-arming rechecks the wait under the provider advisory lock and commits before releasing it. A retained CapacityWait task stays Working on its lease, is excluded from MaxConcurrentTasks while waiting, and returns to a freed slot ahead of newly queued work. Confirmation is a matching UserPrompt, never enqueue or screen-only Delivered. Exhausted Attention (`CapacityRecoveryExhausted`) is derived from the durable wait. A task's prior waits end when its candidate changes (wall requeue, dispatch-time rewalk, routing-blocked resume) or it is cancelled (`Superseded`, reason `requeued:*` / `rerouted-at-dispatch` / `resumed-routing-blocked` / `task-canceled`); the dispatch gate matches a wait only on the task's current kind; a redeemed dispatch receipts its wait (`StartAccepted`) so the transcript progresses it; reconciliation cancels waits whose session or task has ended (CARD-0481).
...
- **A brand-new working directory opens Claude on the TRUST DIALOG, and everything downstream reads that as a healthy, idle session** (REQUIREMENT, live miss 2026-08-16, CARD-0047's standing check interpreter): Claude's trust prompt ("Is this a project you created or one you trust?") is per-cwd in `~/.claude.json`, so the FIRST launch into any directory nobody has run Claude in — a purpose-built scratch dir, a fresh worktree — parks the TUI on a modal and waits. That state is invisible to every signal we have: a modal makes NO output, so the quiet-period ready detector calls it READY; the composer then swallows every write; delivery verification correctly reports `NoComposerEvidence`; and the always-on kill restarts the session **into the same directory, onto the same dialog**. The specialist died this way on all seven of its launches, interpreting zero checks, with the deterministic-digest fallback covering for it the whole time. `ClaudeBlockingPromptDetector.ClearStartupTrustPromptAsync` now runs in BOTH Claude adapters' `WaitForReadyAsync`, after the quiet wait (the modal has rendered by then) and before ready is reported. **The trust helper answers only the trust dialog**: choosing the working directory IS the trust decision and the operator already made it, but keying "1" into a tool-permission modal would approve a tool call nobody authorised, and the generic numbered-menu arm is too weak a shape-match to type into a live session on — those are logged and left standing. Answering persists in `~/.claude.json`, so it fires once per directory, ever. The deny-all PreToolUse hook was NOT the cause and did not need narrowing (spec §4.2's open question, settled). Pinned by `ClaudeStartupTrustPromptTests` (unit) and `RunnerClaudeAdapterTrustPromptTests` (through the production adapter); the real-Claude shape stays in `ClaudeTrustPromptCanaryTests`. **CARD-0390 / CARD-0391:** Claude 2.1.258 paints an unnumbered highlighted list whose default is **No, exit** (ASCII `>`, not a numbered `1.`/`2.`). The answerer moves the highlight (`j`, then Down, then Ctrl+N) and sends Enter **only after the screen shows "Yes, I trust this folder" highlighted** — never Enter while No is highlighted. Measured 2026-09-05: `j` then Enter clears the dialog on modern ConPTY; inbox conhost delivers the bytes but the Select binding never fires. Linked worktrees are trusted through the canonical (main) repo root, so most `C:\Antiphon\worktrees\card-task-*` seats never see the dialog; non-git scratch dirs such as `C:\logs\antiphon\diagnose` do. `StandingSpecialistProvisioner` seeds `projects[<forward-slash key>].hasTrustDialogAccepted` at provision so those seats skip it. A dialog that still will not clear fails the launch as `LaunchBlock: TrustDialogNotCleared` (cwd, layout, and the seed-key remedy in `FailureReason`), not the generic "Agent process did not become ready."

**CARD-0449:** Both Claude adapters also resolve the narrowly parsed startup effort-default
picker through `ClaudeStartupReadiness`. The resolved `--effort` selects the matching row;
...
Disabling the probe still runs the gate. Other permission/choice modals retain their
no-input, probe-skipped pass-through. Scripted checks prove the control flow and local
ConPTY wiring; real-provider picker acceptance is recorded separately in CARD-0449's
verification report.
...
- **A working session that makes no progress is a stall row, not a kill** (CARD-0153): `TaskProgressStalled` is detection only — Warning at 30 minutes of looping rows, Error at 90, Critical only if channel-bound. Never auto-kills, never auto-compacts, never retypes. Recover uncommitted work with `pwsh -File scripts/checkpoint-task.ps1 -TaskId <id> [-Push]` (WIP commit, no kill), then decide: dismiss, `delegate.ps1 -Cancel`, or re-dispatch naming the checkpoint SHA. Grok's `contextFullness` badge is Grok's own numbers (single-call `turn_completed.usage.inputTokens` / `auto_compact_completed.tokens_after`) against the measured 500K window (CARD-0157); a multi-call turn does not move it. The idle-compaction sweep never types `/compact` into Grok (`RefocusCompact` is Unknown). CARD-0079 is the narrow exception for one Working shape only: an AlwaysOn Claude Check seat, an explicit `(auto)` CompactBoundary, its synthetic continuation, and silence of at least `Delegation:CheckCompactionContinuationWaitMinutes` (default 10; zero disables the next stop or resume). The coordinator confirms a fresh tail, commits the attempt, calls `POST /sessions/{id}/stop-compaction-continuation`, then strict-resumes that same conversation. The episode stays open until a new useful Check and the whole caller UserPrompt; it does not authorize a stop of any other turn. An unresolved legacy Check publication keeps its interpreter task, session and transcript, and a confirmed publication keeps its identity so a later capture cannot mint a second note.
<!-- CARD-0254 preserved source ends -->

### Gotcha #57

- **`/remote-control` is not idempotent — a live bridge opens a blocking management menu, and enqueue is not a prompt** (CARD-0292, CARD-0514): Armed/Unarmed/Unknown is classified immediately before any automatic write, on every launch mode. Armed or a current menu suppresses arming; Unknown withholds automatic RC and records monitoring degradation. Automatic arm is a typed maintenance row (System at boot, Supervision from health) with accepted generation, at most one Enter, never a `DeliveryVerdict.Delivered` or synthetic UserPrompt. Health no longer re-arms or kills an armed/zero-connection bridge. Exact-menu observation is independent of working/idle and of RC connection-watch; automatic Esc is idle-gated, one per episode, and verified by two clear grids. Open `RemoteControlModalEpisode` rows are the Attention source (`AttentionKind.RemoteControlModal`); they block ordinary queue bytes until proven clear. Automatic body/Enter/Esc use `POST /sessions/{id}/conditional-input` when the runner advertises `conditionalMaintenanceInputV1`; there is no fallback to raw `/input`. `queue-operation` JSONL is still never confirmation. CARD-0354: Claude's `/remote-control` is a declared local command (`WritesUserPrompt: false`).
...
- **Codex unobservable submit evidence is Working or emptied-composer sustained `PostEvidenceSettleMs` (500 ms)** (CARD-0299): a single empty/ghost/MCP-spinner snapshot is not a submit. At the unobservable deadline, a body still visible in the composer (and no Working latched) is `NoSubmitOutput`, not degraded Sent. **CARD-0574 supersedes the CARD-0299 S3 ready gate:** absence of the MCP line, quiet, or `CodexBootStatusMaxWaitMs` expiry must not authorize work. Ready requires a positively identified loaded-model empty-composer model/effort/cwd-footer layout, settled for `CodexReadyQuietPeriodMs`, fail-closed at `CodexReadyMaxWaitMs`. `CodexBootStatusMaxWaitMs` is now a diagnostic threshold only. A cold Codex first-delivery `NoSubmitOutput` (null baseline, origin Delegation, attempts 1, Dispatched task) still records `BootWedged` (44) *after typing*; a startup that never received work must not consume that relaunch. The named/card-launch boot prompt (`CodexSubmitConfirmation`) degrades to the same Working / body-still-visible look when no transcript ever binds.
...
- **A sibling warning does not select that sibling as the new worktree's base** (CARD-0215/CARD-0508). Provisioning uses its resolved dispatch-base decision. A Plan commit left only on `feat/card-task-<planId>` is absent from a later sibling's history until it reaches that base. Land the Plan before dispatching work that requires it. The guard checks kept Succeeded/Blocked siblings against the base using patch containment, holds active lands, and otherwise captures the reduced warnings described above. A `Landed` outcome carrying `unlanded-sibling=<id>:<branch>` still identifies stranded same-card work. Do not repurpose `mergeTargetRef` as an implicit sibling-chain selector: it is also the merge-back target. Pinned by `AgentTaskDispatchBaseGuardTests`, `WorktreeBaseSelectionTests`, `AgentTaskLandStageOutcomeTests` and `DelegationWorktreeTests`.
...
- **A reply to a Blocked task records `AgentTask.RepliedAtSequence`; settlement refuses any turn whose prompt is at or below it** (REQUIREMENT, CARD-0348; live miss 2026-09-02/03, tasks `a571d6c1`, `42bb5ffb`, `fbd37f59`). `AnswerAsync`'s Blocked→Working branch stamps `MAX(TranscriptEntries.Sequence)` for the delegate session onto the row in the same `SaveChanges` as the `Replied` event. `ExtractMarkedTurnAsync` then refuses a turn whose **prompt** (not the `TurnEnd` boundary) is at or below that watermark, and arm 0 of `SettleDeferredReportsAsync` skips a boundary at or below it so it stops re-invoking settlement every 5 s while the delegate answers. The gate is on the prompt because a promptless restart-marker boundary after the reply would still walk back to the old marked brief. The CARD-0241 in-turn overlay reply stamps `RepliedAt` only — that turn's prompt legitimately predates the reply, and stamping the sequence would refuse the brief's own settlement. `OnTurnEndAsync` stays closed to `Blocked`. Cleared on requeue (sequences are per session). Pinned by `AgentTaskSettlementRaceTests` (answered blocked task is not re-blocked; answer turn settles; stale done after a conflict reply does not re-Succeed) and `AgentTaskDeliveryWatchdogTests.arm_0_skips_a_boundary_at_or_below_the_reply_watermark`.
...
Settlement and the deferred-report sweep share one selector. They walk `TurnEnd` rows from the newest. A Grok `<system-reminder>` background-completion envelope and a queue-backed rules refresh open their own turn: a marked brief does not take that boundary and waits for its own `TurnEnd`, because publishing it would treat the acknowledgement as the report. A Claude `<task-notification>` acknowledgement whose assistant text lacks this task's closing report token is skipped the same way when its in-turn owner is marked. Command and compaction records, including auto-compaction's continuation prompt with no command wrapper, sit inside the brief's turn, so an eligible marked owner settles on that boundary. The selected boundary keeps its own `ApiCallId`. A newer real prompt without this task's marker is a barrier, including a UserPrompt or QueuedUserPrompt that shares a skipped housekeeping turn (it sits after the next-older TurnEnd). An eligible unmarked owner of a Claude notification is that same barrier when the notification answer has no closing token. A prompt at or below `RepliedAtSequence`, or with a known native timestamp at or before dispatch, cannot own the current attempt. A Claude notification whose own assistant text carries this task's closing token can still settle when the walk back reaches the marked brief and subagent grace allows it. That same allowlist keeps a completion reminder from superseding an API-error wall. The delivery watchdog re-runs that verdict after catch-up. Arm 2 fails only when the fresh verdict is still uncorrelated and an eligible incident already exists. That failure commit compares status, concurrency token, session, dispatch time and reply watermark; zero rows leaves the current attempt alone. Pinned by `TaskReportHousekeepingTests`, `TranscriptPromptSpanTests` (`C714_Grok_completion_is_housekeeping`, `C714_Grok_housekeeping_does_not_count_as_started`), `AgentTaskReplyIntegrationTests` `C714_*`, `AgentTaskDeliveryWatchdogTests` `C714_*`, and `CodexTranscriptNormalizerTests.C714_Metadata_cannot_create_a_housekeeping_prompt`.
