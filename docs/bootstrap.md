...
- **AppHost watchdog** — per-user Scheduled Task **"Antiphon AppHost Watchdog"**, every **2 minutes**. `scripts/watchdog-apphost.ps1` probes `http://localhost:17202/health` and `http://localhost:17203/` over HTTP (not TCP listen: Aspire's `dcpctrl` owns both published ports), confirms three failed rounds ~15 s apart, then calls `scripts/restart-apphost.ps1` so the session-runner on 17204 is never touched. Cooldown 10 min / flap cap 3 restarts per 60 min; log `logs/watchdog-apphost.log`. It skips while **either** `logs/apphost.launch.lock` or `logs/apphost.restart.lock` is held, re-checks both **immediately before** restarting (the check at the top of a fire is up to ~60 s of probing old), and does not stamp a restart when `restart-apphost.ps1` exits 3 — a refusal killed nothing and must not spend flap budget (CARD-0075). Delayed 15 minutes after logon so it does not kill the logon launch. Opt out of registration with `install-autostart.ps1 -NoWatchdog`.
