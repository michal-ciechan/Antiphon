...
origin into `.antiphon/card0490-live.json`. The image pins Grok 1.0.40 and Claude Code 2.1.280. The script starts an
...
- **A `git push origin <sha>:master` from a worktree does NOT move the main checkout's local `master` branch — the very next `restart-apphost.ps1` silently rebuilds and serves the OLD code, reporting "healthy"** (CARD-0358, 2026-09-04). `restart-apphost.ps1` builds from whatever `C:\src\Antiphon`'s working tree currently has on disk; it does not fetch or pull. Pushing a commit straight to `origin/master` from elsewhere (a worktree, a different clone) advances GitHub's ref but leaves the main checkout's own `master` pointer untouched — pushing to a remote ref never updates your local branch, regardless of which directory the push ran from. A health check after the restart proves the process came up, not that it is running the code you think it is: a stale-but-internally-consistent build passes `/health` exactly like a fresh one. This is also the actual cause behind most instances of `-Land` refusing with `"origin/master moved ahead of local master"` — that refusal is usually correct (the main checkout genuinely is behind), and re-running `-Land` blindly, or working around it with another out-of-band push, compounds the staleness instead of fixing it. **The fix**: after ANY push to `origin/master` that did not go through the normal `-Land` pipeline (which lands into the main checkout itself), run `git pull --rebase` in `C:\src\Antiphon` *before* restarting — check `git rev-parse HEAD` vs `git rev-parse origin/master` in the main checkout specifically, not a worktree, since worktrees share objects but not branch pointers. After a restart whose code matters, don't just trust "healthy" — `restart-apphost.ps1` now requires `GET /api/version` to equal source-root HEAD (exit 5 and a retained restart lock on mismatch; CARD-0495) and `delegate.ps1 -Land` refuses unless that body advertises exact `land-v2`. Spot-check the required feature as well when the checkout has tracked edits. More generally: any assumption about what state the running system is in (checkout HEAD, whether a restart picked up a change, whether a service is authenticated) should be checked directly and immediately, not inferred from a downstream signal like a passing health check or an absence of errors. The API-level signature of this skew is a 400 `The JSON value could not be converted to …CreateAgentTaskRequest. Path: $.<field>` for a value the script's `ValidateSet` accepts; `delegate.ps1` now prints a `diagnosis:` line relating the served SHA to HEAD (CARD-0493, 2026-09-12).
...
- **A hard-killed git child orphans `.git/index.lock`** (CARD-0543). `GitWorkspaceService` now
  ...
  running; `Remove-Item` the path from the hold/refusal detail once the census is clear.
  Simulate only in a scratch repo (`git rev-parse --path-format=absolute --git-path index.lock`,
  then `File.WriteAllBytes` + `File.SetLastWriteTimeUtc`); never touch `C:\src\Antiphon\.git`.
