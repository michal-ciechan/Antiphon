...
switch ($answer) {
    ...
    'deadline_unreachable' {
        ...
        Add-Content -LiteralPath $log -Value ('FIRST_FAILURE_AT ' + [Diagnostics.Stopwatch]::GetTimestamp()) -Encoding ASCII
        return @{ Status = 0; Body = ''; Error = 'connection refused (shim)' }
    }
    'deadline_notfound' {
        Start-Sleep -Milliseconds 1100
        Add-Content -LiteralPath $log -Value ('FIRST_FAILURE_AT ' + [Diagnostics.Stopwatch]::GetTimestamp()) -Encoding ASCII
