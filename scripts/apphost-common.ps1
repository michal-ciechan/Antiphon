...
function Get-AppHostLock {
    ...
    if ($info.StampUtc) {
        $info.AgeMinutes = ([datetime]::UtcNow - $info.StampUtc.ToUniversalTime()).TotalMinutes
    }
...
function Test-AppHostLockActive {
    ...
    if ($null -ne $lock.AgeMinutes) {
        $age = '{0:N1} min old' -f $lock.AgeMinutes
    } else {
        $age = 'age unknown'
    }
    if ($lock.ProcessId) {
        return ("{0} held by PID {1} ({2}; {3})" -f $Label, $lock.ProcessId, $age, $Path)
    }
    return ("{0} present but unreadable ({1}; {2})" -f $Label, $age, $Path)
}
