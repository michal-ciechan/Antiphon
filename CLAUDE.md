...
| 17284 | Aspire dashboard UI             |
| 17285 | OTLP telemetry endpoint         |
...
- **Starting the AppHost**: Use `dev-aspire.ps1` via `Start-Process wt -ArgumentList @('new-tab','--title','Antiphon AppHost','--','pwsh','-NoLogo','-NoExit','-File','C:\src\antiphon\dev-aspire.ps1')` so the AppHost gets its own terminal tab and is not attached to the tool session. Running with `-NoNewWindow` kills the AppHost when the tool session ends. AppHost output is also tee'd to `logs/apphost.log`.
