...
- **Multi-line input to an agent TUI MUST be bracketed-paste wrapped** (REQUIREMENT, live miss 2026-07-29): any programmatic write of a multi-line body into a Claude session must be wrapped in `\e[200~ … \e[201~` with the submitting `\r` as a separate write — `SessionMessageQueueService.DeliverAsync` does this; any new code path that types into a session must too. Raw multi-line writes get chunked by ConPTY and fragment at line breaks (a 2.4 KB Telegram message reached the agent as only its final fragment). Contract pinned by `FakeClaudeContractTests` (fakeclaude models the hazard for unbracketed >512-char multi-line bursts) and `SessionMessageQueuePtyIntegrationTests.Large_multiline_channel_body_submits_as_one_intact_turn`.
