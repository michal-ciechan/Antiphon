...
- **Starting the AppHost**: Use `scripts/start-apphost.ps1` (via `Start-Process pwsh -ArgumentList "-File","scripts/start-apphost.ps1" -WindowStyle Hidden`) instead of `dotnet run` directly. Running with `-NoNewWindow` attaches the AppHost to the tool's console — when the tool session ends, CTRL_CLOSE_EVENT kills it. The wrapper gives it an isolated hidden console.
